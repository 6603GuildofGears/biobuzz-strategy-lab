...
const FROM_MANUAL = [
  ...
  "PIN: holding an opponent still against a wall or FIELD element for more than 3 s is a MAJOR FOUL (20 pts to them), plus another every 3 s (G421). It applies to every robot, and every robot backs off before 3 s.",
...
const ASSUMPTIONS = [
  ...
  "Launching is real projectile physics: the ball leaves at the Launcher height (about 14.5 in) and must climb to the CELL opening, so shot speed sets the longest shot. Robots must shoot from at least 1.5 ft out. Accuracy slider = accuracy up close. At the robot's longest range it's half that.",
  ...
  "Intake: robots drive over balls instead of stopping. Any ball the robot wants that enters the intake's mouth (the Intake width slider) comes in, at up to about 1 ft of travel per intake time. Driving into a ball faster than that knocks it away. A ball tucked between a FLOWER and the wall can only be reached if the intake is wide enough to get it without the frame hitting the FLOWER; otherwise robots leave it.",
  "Defense: the defender reads where each opponent is going to shoot and gets in front of it, between it and the CELL, updating every 0.3 s. Shots leave the launcher (Launcher height slider) and climb steeply, so a robot pressed against the shooter's front and taller than the ball's path there is in the way. Shooters keep shooting, and each blocked shot is knocked down 60% of the time (slider). When picking a spot, shooters prefer ones no opponent is blocking. Knocked-down balls glance off to one side. Being pushed also slows a robot and makes it less accurate (slider).",
  "Collisions: robots are rectangles turned to their heading, so a robot at an angle only takes up the room its frame really does.",
  "AUTO: robots stay entirely on their own half (columns A to C for red, D to F for blue, G402). They plan routes that way and treat the center line as a wall until AUTO ends.",
